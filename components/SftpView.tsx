        ...
        >
            ...
            <div className="h-10 px-4 flex items-center gap-2 border-b border-border/40 bg-secondary/20">
                ...
                {isEditingPath ? (
                    <div className="relative flex-1">
                        <Input
                            ...
                            onChange={e => { setEditingPathValue(e.target.value); setShowPathSuggestions(true); setPathSuggestionIndex(-1); }}
                            ...
                            >
                                {pathSuggestions.map((suggestion, idx) => (
                                    <button
                                        ...
                                        onMouseDown={(e) => { e.preventDefault(); handlePathSubmit(suggestion.path); }}
                                    >
                ...
                <div className="ml-auto flex items-center gap-1">
                    <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={() => setShowNewFolderDialog(true)}>
            ...
            >
                <div className="flex items-center gap-1 cursor-pointer hover:text-foreground relative pr-2" onClick={() => handleSort('name')}>
                    <span>Name</span>
                    {sortField === 'name' && <span className="text-primary">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                    <div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/50 transition-colors" onMouseDown={(e) => handleResizeStart('name', e)} />
                </div>
                <div className="flex items-center gap-1 cursor-pointer hover:text-foreground relative pr-2" onClick={() => handleSort('modified')}>
                    <span>Modified</span>
                    {sortField === 'modified' && <span className="text-primary">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                    <div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/50 transition-colors" onMouseDown={(e) => handleResizeStart('modified', e)} />
                </div>
                <div className="flex items-center gap-1 cursor-pointer hover:text-foreground relative pr-2 justify-end" onClick={() => handleSort('size')}>
                    {sortField === 'size' && <span className="text-primary">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                    <span>Size</span>
                    <div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary/50 transition-colors" onMouseDown={(e) => handleResizeStart('size', e)} />
                </div>
                <div className="flex items-center gap-1 cursor-pointer hover:text-foreground justify-end" onClick={() => handleSort('type')}>
                    {sortField === 'type' && <span className="text-primary">{sortOrder === 'asc' ? '↑' : '↓'}</span>}
                    <span>Kind</span>
                </div>
            </div>
            ...
            <div
                ref={fileListRef}
                className={cn("flex-1 min-h-0 overflow-y-auto relative", isDragOverPane && "ring-2 ring-primary/30 ring-inset")}
            >
                                    ...
                                    <SftpFileRow
                                        ...
                                        onSelect={(e) => {
                                            ...
                                            if (e.shiftKey && lastSelectedIndexRef.current !== null) {
                                                ...
                                                const selectedFileNames = sortedDisplayFiles.slice(start, end + 1).filter(f => f.name !== '..').map(f => f.name);
                                ...
                                {entry.name !== '..' && (
                                    <ContextMenuContent>
                                        ...
                                        <ContextMenuItem onClick={() => {
                                            const files = pane.selectedFiles.has(entry.name) ? Array.from(pane.selectedFiles) : [entry.name];
                ...
                <DialogContent className="max-w-sm">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input value={newFolderName} onChange={e => setNewFolderName(e.target.value)} placeholder="Enter folder name" onKeyDown={e => e.key === 'Enter' && handleCreateFolder()} autoFocus />
                        </div>
                    </div>
                    <DialogFooter>
                ...
                <DialogContent className="max-w-sm">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input value={renameName} onChange={e => setRenameName(e.target.value)} placeholder="Enter new name" onKeyDown={e => e.key === 'Enter' && handleRename()} autoFocus />
                        </div>
                    </div>
                    <DialogFooter>
            ...
            <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        ...
                        <DialogDescription>This action cannot be undone. The following will be deleted:</DialogDescription>
                    </DialogHeader>
            ...
            <SftpHostPicker
                ...
                hostSearch={hostSearch}
                onHostSearchChange={setHostSearch}
                onSelectLocal={() => { onDisconnect(); onConnect('local'); }}
                onSelectHost={(host) => { onDisconnect(); onConnect(host); }}
            />
        </div>
    );
};
...
const SftpViewInner: React.FC<SftpViewProps> = ({ hosts, keys }) => {
    ...
    const handleDragEnd = useCallback(() => { setDraggedFiles(null); }, []);

    const handleCopyToOtherPaneLeft = useCallback((files: { name: string; isDirectory: boolean }[]) => sftp.startTransfer(files, 'left', 'right'), [sftp.startTransfer]);
    const handleCopyToOtherPaneRight = useCallback((files: { name: string; isDirectory: boolean }[]) => sftp.startTransfer(files, 'right', 'left'), [sftp.startTransfer]);
    const handleReceiveFromOtherPaneLeft = useCallback((files: { name: string; isDirectory: boolean }[]) => sftp.startTransfer(files, 'right', 'left'), [sftp.startTransfer]);
    const handleReceiveFromOtherPaneRight = useCallback((files: { name: string; isDirectory: boolean }[]) => sftp.startTransfer(files, 'left', 'right'), [sftp.startTransfer]);
    ...
    const containerStyle: React.CSSProperties = isActive ? {} : { visibility: 'hidden', pointerEvents: 'none', position: 'absolute', zIndex: -1 };

    return (
        <div className={cn("absolute inset-0 min-h-0 flex flex-col", isActive ? "z-20" : "")} style={containerStyle}>
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 min-h-0 border-t border-border/70">
                <div className="relative border-r border-border/70">
                    <SftpPaneView
                        side="left" pane={sftp.leftPane} hosts={hosts} filteredFiles={leftFilteredFiles}
                        onConnect={handleConnectLeft} onDisconnect={handleDisconnectLeft} onNavigateTo={handleNavigateToLeft}
                        onNavigateUp={handleNavigateUpLeft} onRefresh={handleRefreshLeft} onOpenEntry={handleOpenEntryLeft}
                        onToggleSelection={handleToggleSelectionLeft} onRangeSelect={handleRangeSelectLeft} onClearSelection={handleClearSelectionLeft}
                        onSetFilter={handleSetFilterLeft} onCreateDirectory={handleCreateDirectoryLeft} onDeleteFiles={handleDeleteFilesLeft}
                        onRenameFile={handleRenameFileLeft} onCopyToOtherPane={handleCopyToOtherPaneLeft} onReceiveFromOtherPane={handleReceiveFromOtherPaneLeft}
                        onEditPermissions={handleEditPermissionsLeft} draggedFiles={draggedFiles} onDragStart={handleDragStart} onDragEnd={handleDragEnd}
                    />
                </div>
                <div className="relative">
                    <SftpPaneView
                        side="right" pane={sftp.rightPane} hosts={hosts} filteredFiles={rightFilteredFiles}
                        onConnect={handleConnectRight} onDisconnect={handleDisconnectRight} onNavigateTo={handleNavigateToRight}
                        onNavigateUp={handleNavigateUpRight} onRefresh={handleRefreshRight} onOpenEntry={handleOpenEntryRight}
                        onToggleSelection={handleToggleSelectionRight} onRangeSelect={handleRangeSelectRight} onClearSelection={handleClearSelectionRight}
                        onSetFilter={handleSetFilterRight} onCreateDirectory={handleCreateDirectoryRight} onDeleteFiles={handleDeleteFilesRight}
                        onRenameFile={handleRenameFileRight} onCopyToOtherPane={handleCopyToOtherPaneRight} onReceiveFromOtherPane={handleReceiveFromOtherPaneRight}
                        onEditPermissions={handleEditPermissionsRight} draggedFiles={draggedFiles} onDragStart={handleDragStart} onDragEnd={handleDragEnd}
                    />
                </div>
            </div>
                ...
                <div className="border-t border-border/70 bg-secondary/80 backdrop-blur-sm shrink-0">
                    <div className="flex items-center justify-between px-4 py-2 text-xs text-muted-foreground border-b border-border/40">
                        <span className="font-medium">
                            Transfers
                            {sftp.activeTransfersCount > 0 && <span className="ml-2 text-primary">({sftp.activeTransfersCount} active)</span>}
                        </span>
                        {sftp.transfers.some(t => t.status === 'completed' || t.status === 'cancelled') && (
                            <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={sftp.clearCompletedTransfers}>Clear completed</Button>
                        )}
                    </div>
                    <div className="max-h-40 overflow-auto">
                        {visibleTransfers.map(task => (
                            <SftpTransferItem key={task.id} task={task} onCancel={() => sftp.cancelTransfer(task.id)} onRetry={() => sftp.retryTransfer(task.id)} onDismiss={() => sftp.dismissTransfer(task.id)} />
                        ))}
                    </div>
                </div>
            )}

            <SftpConflictDialog conflicts={sftp.conflicts} onResolve={sftp.resolveConflict} formatFileSize={sftp.formatFileSize} />
                ...
                onSave={(file, permissions) => {
                    if (permissionsState) {
                        const fullPath = sftp.joinPath(
                            permissionsState.side === 'left' ? sftp.leftPane.connection?.currentPath || '' : sftp.rightPane.connection?.currentPath || '',
                            file.name
                        );
...
const sftpViewAreEqual = (prev: SftpViewProps, next: SftpViewProps): boolean => prev.hosts === next.hosts && prev.keys === next.keys;
